...
!           type (hecmwST_local_mesh)       :: hecMESH       !< mesh information
           ...
           if( cdsys_ID>0 ) then
                 if( isCoordNeeds(g_LocalCoordSys(cdsys_ID)) ) then
                    ...
                    call ppohFEM_get_node_coord(idx_mesh, ik, XYZ)
                    coords(1,:)= XYZ(:)+fstrSOLID%unode(3*ik-2:3*ik) + fstrSOLID%dunode(3*ik-2:3*ik)

                    ik=g_LocalCoordSys(cdsys_ID)%node_ID(2)
                    call ppohFEM_get_node_coord(idx_mesh, ik, XYZ)
                    coords(2,:)= XYZ(:)+fstrSOLID%unode(3*ik-2:3*ik) + fstrSOLID%dunode(3*ik-2:3*ik)

                    ik=g_LocalCoordSys(cdsys_ID)%node_ID(3)
                    call ppohFEM_get_node_coord(idx_mesh, ik, XYZ)
                    if(ik>0) coords(3,:)= XYZ(:)+fstrSOLID%unode(3*ik-2:3*ik) + fstrSOLID%dunode(3*ik-2:3*ik)
                 endif
           endif
        end subroutine
