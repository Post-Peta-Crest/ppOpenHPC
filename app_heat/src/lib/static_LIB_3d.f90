...
!---------------------------------------------------------------------
    ...
    DO LX = 1, ppohFEM_get_num_quadpoints_of_element(etype)

      CALL ppohFEM_get_quadpoints_of_element( etype, LX, naturalCoord(:) )
      CALL ppohFEM_get_global_deriv(etype, nn, naturalcoord, elem, det, gderiv)
      ...
      IF( PRESENT(temperature) ) THEN
        CALL ppohFEM_get_shape_func(etype, naturalcoord, spfunc)
      ...
      wg = ppohFEM_get_weight(etype, LX)*det
...
! INTEGRATION OVER SURFACE
        ...
        DO IG2=1,NumOfQuadPoints( SURTYPE )
            ...
            WG=ppohFEM_get_weight( SURTYPE, IG2 )
...
!---------------------------------------------------------------------
    ...
    DO LX = 1, NumOfQuadPoints(etype)
      ...
      VOLUME_C3 = VOLUME_C3+ppohFEM_get_weight(etype, LX)*det
