...
int main(void)
{
	...
	//if (iauto == 1){
       ...
       OAT_ENDTUNESIZE = 500;
       OAT_SAMPDIST = 100;
	 OAT_ATexec(OAT_DYNAMIC, OAT_DynamicRoutines,A,B,C,in);
	//}else{
...
MatMul(double A[N][N],double B[N][N], double C[N][N], int n)
{
	...
	OAT_SetParm(3,"MyMatMul",n,&OAT_iusw1_MyMatMul,A,B,C,n);
	OAT_DynamicMyMatMul(C,A,B,n,OAT_iusw1_MyMatMul);
//	#pragma OAT dynamic unroll (i) region start
//	#pragma OAT name MyMatMul
//	#pragma OAT fitting dynamicdspline
//	#pragma OAT varied (i) from 1 to 16
//	#pragma OAT debug (pp)
	if(OAT_DEBUG >= 1){
		printf("myid: %d\n",myid);
		printf("Dynamic Routine: MyMatMul\n");
		printf("iusw1: %d\n",OAT_iusw1_MyMatMul);
	}
//	for(i = 0 ; i < n ; i++){
//		for(j = 0 ; j < n ; j++){
...
//		}
//	}
//	#pragma	#pragma OAT dynamic unroll (i) region end
	return 0;
}
