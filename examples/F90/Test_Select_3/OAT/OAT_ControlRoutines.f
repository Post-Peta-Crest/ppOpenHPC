...
c     ==== All routines
      if (OAT_TYPE .eq. 0) then
        OAT_Routines(1:9) = 'BlkMatVec'
        iusw1_BlkMatVec_flag = 0
      endif
...
c     ==== Before Execution-invocation Optimization
      if (OAT_TYPE .eq. 2) then
        cp = index(OAT_Routines,'BlkMatVec')
        if (cp .ne. 0) then
          cp = cp + len('BlkMatVec')
          ...
          if (cp .ge. len(OAT_Routines)) then
            call OAT_ATexecStaticBlkMatVec(OAT_Routines,A,B,C,N)
          endif
        endif
      endif
...
c     ==============================================================
      subroutine OAT_ATexecStaticBlkMatVec(OAT_Routines,A,B,C,N)
      character*9 OAT_Routines
      real*8 A(N,N)
      real*8 B(N,N)
      real*8 C(N,N)
...
!          do iloop_iter=1, OAT_MAXSAMPITER
            ...
            call OAT_StaticBlkMatVec(A, B, C, iloop_n, iusw1)
