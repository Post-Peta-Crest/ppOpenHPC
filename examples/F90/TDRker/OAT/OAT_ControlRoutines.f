...
c     ==== All routines
      if (OAT_TYPE .eq. 0) then
        OAT_Routines(1:9) = 'MGSKernel'
        iusw1_MGSKernel_flag = 0
      endif
...
c     ==== Install Optimization
      if (OAT_TYPE .eq. 1) then
        ...
        if (index(OAT_Routine,'MGSKernel') .ne. 0) then

		if (iusw1_MGSKernel_flag .eq. 0) then

		  iusw1_MGSKernel_flag = 1
...
c         -----------------------------------------
          if (myid .eq. 0) then
            open(21, status = 'old',
     &         file = 'OAT_InstallMGSKernelParam.dat',
     &         action = 'read', pad= 'yes', err =102)

c           --- Seek object file
            read(21, *) cbuf
            do while (index(cbuf,'MGSKernel') .eq. 0)
              read(21, *) cbuf
            enddo
...
c             --- Find parameter
              read(UNIT=21, FMT='(A,A)', END=100) cbuf
              do while (index(cbuf, 'MGSKernel_I') .eq. 0)
                 read(UNIT=21, FMT='(A,A)', END=100) cbuf
              enddo
c             -------------------------------------------
...
c             ---- find space
              oat_i = index(cbuf, 'MGSKernel_I')+11
              do while(cbuf(oat_i:oat_i) .eq. ' ')
                oat_i = oat_i + 1
              enddo
c             ---------------------------------
...
c     === OAT_ATexec
c     ============================================================
      subroutine OAT_ATexec(OAT_TYPE, OAT_Routines,local_length_y,u_x,mu
     &,y_k,local_length_x,A,init_x,init_y,u_y,x_k,nx,ny)
      integer   OAT_TYPE
      character*9 OAT_Routines
...
c     ==== Install Optimization
      if (OAT_TYPE .eq. 1) then
        cp = index(OAT_Routines,'MGSKernel')
        if (cp .ne. 0) then
          cp = cp + len('MGSKernel')
          ...
          if (cp .ge. len(OAT_Routines)) then
            call OAT_ATexecInstallMGSKernel(OAT_Routines,local_length_y,
     &u_x,mu,y_k,local_length_x,A,init_x,init_y,u_y,x_k,nx,ny)
          endif
        endif
      endif
...
c     ==============================================================
      subroutine OAT_ATexecInstallMGSKernel(OAT_Routines,local_length_y,
     &u_x,mu,y_k,local_length_x,A,init_x,init_y,u_y,x_k,nx,ny)
      character*9 OAT_Routines
      integer local_length_y
      real*8 u_x(0:nx-1)
...
!          do iloop_iter=1, OAT_MAXSAMPITER
            ...
            call OAT_InstallMGSKernel(local_length_y, u_x, mu, y_k, loca
     &l_length_x, A, init_x, init_y, u_y, x_k, nx, ny, iusw1)
