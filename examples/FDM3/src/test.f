!!====================================================================!!
!!                                                                    !!
!!   Software Name : FDMtest-3 using ppOpen-MATH/VIS-FDM3D            !!
!!         Version : 0.2.0                                            !!
!!                                                                    !!
!!   License:                                                         !!
...
!C-- My Location
      ...
      allocate ( PHI(0:NXP+1, 0:NYP+1, 0:NZP+1, 3))
      allocate (dPHI(0:NXP+1, 0:NYP+1, 0:NZP+1, 3))
...
!C-PPOHVIS_FDM3D-s
      ...
      PRESELM%RESULTS(1)%FREEDOMCOUNT = 3
      PRESELM%RESULTS(1)%LABEL = ""
      PRESELM%RESULTS(1)%LABEL = "PHI"
      ALLOCATE(PRESELM%RESULTS(1)%VALUE(NXP*NYP*NZP*3))
...
!C-- SEND
      ...
      do neib= 1, NEIBPEtot
        ...
        if (NEIBsurf(neib).eq.2) then
          ...
          do j= 1, NYP
            icou= icou + 1
            SENDbuf(iS+icou)= PHI(NXP,j,k,ii)
          enddo
          enddo
        endif
        ...
        if (NEIBsurf(neib).eq.4) then
          ...
          do i= 1, NXP
            icou= icou + 1
            SENDbuf(iS+icou)= PHI(i,NYP,k,ii)
          enddo
          enddo
        endif
        ...
        if (NEIBsurf(neib).eq.6) then
          ...
          do i= 1, NXP
            icou= icou + 1
            SENDbuf(iS+icou)= PHI(i,j,NZP,ii)
          enddo
          enddo
        endif
     ...
     &                  ierr)
      ...
      do neib= 1, NEIBPEtot
        ...
        if (NEIBsurf(neib).eq.2) then
          ...
          do j= 1, NYP
            icou= icou + 1
            PHI(NXP+1,j,k,ii)= RECVbuf(iS+icou)
          enddo
          enddo
        endif
        ...
        if (NEIBsurf(neib).eq.4) then
          ...
          do i= 1, NXP
            icou= icou + 1
            PHI(i,NYP+1,k,ii)= RECVbuf(iS+icou)
          enddo
          enddo
        endif
        ...
        if (NEIBsurf(neib).eq.6) then
          ...
          do i= 1, NXP
            icou= icou + 1
            PHI(i,j,NZP+1,ii)= RECVbuf(iS+icou)
          enddo
          enddo
        endif
      enddo
...
!C===
      ...
      do i= 1, NXP
        dPHI(i,j,k,ii)= DT * (
     &    PHI(i-1,j,k,ii) + PHI(i+1,j,k,ii) + PHI(i,j-1,k,ii) + 
     &    PHI(i,j+1,k,ii) + PHI(i,j,k-1,ii) + PHI(i,j,k+1,ii) 
     &                 - 6.d0*PHI(i,j,k,ii) + Q(i,j,k))
      enddo
      enddo
      enddo
      ...
      do j= 1, NYP
      do i= 1, NXP
        PHI(i,j,k,ii)= PHI(i,j,k,ii) + dPHI(i,j,k,ii)
      enddo
      enddo
      enddo
...
!C===
      ...
      if (Xmax.eq.1) then
        do k= 1, NZP
        do j= 1, NYP
          PHI(NXP+1,j,k,ii)= PHI(NXP,j,k,ii)
        enddo
        enddo
      endif
      ...
      if (Ymax.eq.1) then
        do k= 1, NZP
        do i= 1, NXP
          PHI(i,NYP+1,k,ii)= PHI(i,NYP,k,ii)
        enddo
        enddo
      endif
      ...
      if (Zmax.eq.1) then
        do j= 1, NYP
        do i= 1, NXP
          PHI(i,j,NZP+1,ii)= PHI(i,j,NZP,ii)
        enddo
        enddo
      endif
...
!C-PPOHVIS_FDM3D-s
        ...
        do i= 1, NXP
        do ii= 1, 3
          L = 3*(NXP*NYP*(K-1) + NXP*(J-1) + I - 1) + ii
          PRESELM%RESULTS(1)%VALUE(L)= PHI(i,j,k,ii)
        enddo
        enddo
        enddo
        enddo
