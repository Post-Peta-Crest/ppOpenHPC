...
!        end if
        ...
        if(hecMESH%my_rank==0) write(*,*) 'app_struct Completed !!'
...
subroutine fstr_init_file
        ...
        if( flag == 0 ) then
          ...
          write( dbgfileNAME, '(a,a)') 'struct.dbg.', trim(adjustl(s))
          ...
          if( nprocs > limit ) then
            ...
            write( dbgfileNAME, '(a,a,a,a,a)') 'DBG/TRUNK', trim(adjustl(r)), '/', 'struct.dbg.', trim(adjustl(s))
            dbgfileNAME = adjustl(dbgfileNAME)
            ...
            call hecmw_ctrl_make_subdir( logfileNAME, stat )
            if( stat /= 0 ) call fstr_setup_util_err_stop( '### Cannot create directory' )
            write( dbgfileNAME, '(a,a,a)') 'DBG/', 'struct.dbg.', trim(adjustl(s))
            dbgfileNAME = adjustl(dbgfileNAME)
            call hecmw_ctrl_make_subdir( dbgfileNAME, stat )
            if( stat /= 0 ) call fstr_setup_util_err_stop( '### Cannot create directory' )
          end if
        end if
        stafileNAME = 'struct.sta'
        msgfileNAME = 'struct.msg'
        ...
        if( myrank == 0) then
                ...
                write(IMSG,*) ':========================================:'
                write(IMSG,*) ':**   BEGIN Structural Analysis        **:'
                write(IMSG,*) ':========================================:'
