...
integer, parameter :: idx_mesh = 1 !DEBUG currently use only one mesh index
integer, parameter :: idx_mat  = 1 !DEBUG currently use only one matrix index
...
subroutine fstr_Newton( cstep, hecMESH, hecMAT, fstrSOLID, fstrPARAM, &
  ...
  integer, parameter :: idx_mesh = 1 !DEBUG
  ...
  do itemp = fstrSOLID%TEMP_tstep, ttemp, tintl
    ...
    call fstr_ass_load(cstep, idx_mesh, idx_mat, fstrSOLID, fstrPARAM)
    ...
    do iter=1,fstrSOLID%step_ctrl(cstep)%max_iter
      ...
      call fstr_AddSPRING(cstep, sub_step, idx_mesh, idx_mat, fstrSOLID, fstrPARAM)

      ! ----- Set Boundary condition
      call fstr_AddBC(cstep, sub_step, idx_mesh, idx_mat, fstrSOLID, fstrPARAM, fstrMAT, stepcnt)
  ...
  do itemp = fstrSOLID%TEMP_tstep, ttemp
    ...
    loopFORcontactAnalysis: DO WHILE( .TRUE. )
      ...
      do iter = 1, fstrSOLID%step_ctrl(cstep)%max_iter
        ...
        ! ----- Set Boundary condition
        if(paraContactFlag.and.present(conMAT)) then
          call fstr_AddBC(cstep, sub_step, idx_mesh, idx_mat, fstrSOLID, fstrPARAM, fstrMAT, stepcnt, conMAT)
        else
          call fstr_AddBC(cstep, sub_step, idx_mesh, idx_mat, fstrSOLID, fstrPARAM, fstrMAT, stepcnt)
        endif
