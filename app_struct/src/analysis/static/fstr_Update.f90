...
subroutine fstr_Update3D( idx_mesh, fstrSOLID )
        use m_fstr
        use m_static_lib
...
!C** local variables
        ...
        real(kind=kreal)   :: thick, coords(3, 3), XYZ(3)
...
!C set temperature
!C
        allocate( temp(ppohFEM_get_n_node(idx_mesh) ))
        temp = 0.0d0
        do i = 1, fstrSOLID%TEMP_ngrp_tot
                ig = fstrSOLID%TEMP_ngrp_ID(i)
                do j = 1, ppohFEM_get_num_nodes_in_node_group(idx_mesh, ig)
                        temp( ppohFEM_get_node_item_in_node_group(idx_mesh, ig, j) ) = fstrSOLID%TEMP_ngrp_val(i)
                enddo
        enddo
!C
...
!C
        fstrSOLID%QFORCE = 0.0d0
        allocate ( id_spc(ppohFEM_get_n_node(idx_mesh)) )
        id_spc = 0
        do i = 1, fstrSOLID%BOUNDARY_ngrp_tot
                ig = fstrSOLID%BOUNDARY_ngrp_ID(i)
                do j = 1, ppohFEM_get_num_nodes_in_node_group(idx_mesh, ig)
                  id_spc( ppohFEM_get_node_item_in_node_group(idx_mesh, ig, j) ) = 1
                enddo
        enddo
...
!C +-------------------------------+
        do itype = 1, ppohFEM_get_n_elem_type(idx_mesh)
                ...
                if( .not. (ppohFEM_is_etype_solid(ic_type) .or. ic_type == 781 .or. ic_type == 761 .or. ic_type == 641 )) cycle
                nn = ppohFEM_get_max_node_of_elem_type( ic_type )
!C element loop
!$omp parallel default(none), &
!$omp&  private(icel,js,j,nodLOCAL,xx,yy,zz,ecoord,edisp,isect,cdsys_ID,ihead,thick,mixflag,stiff,iflag,coords,XYZ), &
!$omp&  shared(iS,iE,temp,ref_temp,fstrSOLID,id_spc,force,idx_mesh), &
                        ...
                        do j = 1, nn
                                ...
                                if( fstrSOLID%TEMP_ngrp_tot > 0 .or. fstrSOLID%TEMP_irres > 0 ) then
                                 if( isElastoplastic(fstrSOLID%elements(icel)%gausses(1)%pMaterial%mtype) .or. &
                                  ...
                                  tt0(j) = 0.d0
!                                  if( hecMESH%hecmw_flag_initcon==1 ) tt0(j) = hecMESH%node_init_val_item(nodLOCAL(j)) !DEBUG is it required?
                                 endif
                                 tt(j) = fstrSOLID%temperature( nodLOCAL(j) )
                                endif
                                ecoord(1,j) = xx(j)
...
write(*,*) 'element type 641 is currently not supported'
call ppohFEM_abort
!                                isect = hecMESH%section_ID(icel)
!                                ihead = hecMESH%section%sect_R_index(isect-1)
...
!$omp end parallel
        ...
        call ppohFEM_update_3_R( idx_mesh, fstrSOLID%QFORCE, ppohFEM_get_n_node(idx_mesh) )
