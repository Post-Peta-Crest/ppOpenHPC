...
!----------------------------------------------------------------------*
    ...
    if( fstrSOLID%TEMP_ngrp_tot>0 .or. fstrSOLID%TEMP_irres>0 ) then
       if( ndof==3 ) then
          allocate( tnstrain(ppohFEM_get_n_node(idx_mesh)*6) )
          allocate( testrain(ppohFEM_get_n_elem(idx_mesh)*6) )
       else if( ndof==2 ) then
          allocate( tnstrain(ppohFEM_get_n_node(idx_mesh)*3) )
          allocate( testrain(ppohFEM_get_n_elem(idx_mesh)*3) )
       endif
    endif

    if( ndof==3 ) then
          call fstr_NodalStress3D( idx_mesh, fstrSOLID, tnstrain, testrain )
    else if( ndof==2 ) then
    ...
    if( flag==6 ) then
      if( IRESULT==1 .and. &
        (mod(istep,fstrSOLID%output_ctrl(3)%freqency)==0 .or. istep==maxstep) ) then
          call fstr_write_static_result( idx_mesh, fstrSOLID, ppohFEM_get_mat_ndof(idx_mat),  maxstep, istep, tnstrain, testrain, 1 )
      endif
    ...
    if( IRESULT==1 .and. &
        (mod(istep,fstrSOLID%output_ctrl(3)%freqency)==0 .or. istep==maxstep) ) then
          call fstr_write_static_result( idx_mesh, fstrSOLID, ppohFEM_get_mat_ndof(idx_mat), maxstep, istep, tnstrain, testrain, 0 )
    endif
    ...
    if( IVISUAL==1 .and. &
        (mod(istep,fstrSOLID%output_ctrl(4)%freqency)==0 .or. istep==maxstep) ) then
          ...
          call fstr_make_static_result( idx_mesh, fstrSOLID, ppohFEM_get_mat_ndof(idx_mat),  fstrRESULT, tnstrain, testrain )
          call fstr2hecmw_mesh_conv( hecMESH )
          call ppohFEM_visualize_init
          call ppohFEM_visualize_ext( idx_mesh, fstrRESULT, istep, maxstep, interval )
    ...
    if( (mod(istep,fstrSOLID%output_ctrl(1)%freqency)==0 .or. istep==maxstep) ) then
          ...
          call fstr_static_post( fnum, idx_mesh, ppohFEM_get_mat_ndof(idx_mat), fstrSOLID, istep )
    endif
    ...
    if( fstrSOLID%output_ctrl(2)%outinfo%grp_id>0 .and. &
      ...
      do i = 1, ppohFEM_get_num_nodes_in_node_group(idx_mesh, iS)
        iE = ppohFEM_get_node_item_in_node_group(idx_mesh, iS, i)
!        gid = hecMESH%global_node_ID(iE)
        gid = ppohFEM_get_global_node_ID(idx_mesh, iE)
...
!----------------------------------------------------------------------*
  subroutine fstr_static_post( fnum, idx_mesh, ndof, fstrSOLID, istep )
!----------------------------------------------------------------------*
      use m_fstr
      ...
      integer(kind=kint) :: i, j, k,  mdof, ID_area
...
!C*** Show Displacement
      do i = 1, ppohFEM_get_nn_internal(idx_mesh)
        j = ppohFEM_get_global_node_ID(idx_mesh, i)
        if( i==1 ) then
          do k = 1, ndof
...
!C @element
      do i = 1, ppohFEM_get_n_elem(idx_mesh)
        ID_area = ppohFEM_get_rank_assigned_to_element(idx_mesh, i)
        if( ID_area .eq. ppohFEM_comm_get_rank() ) then
          j = ppohFEM_get_global_elem_ID(idx_mesh, i)
          if( ppohFEM_get_local_elem_ID(idx_mesh, i) .eq. 1 ) then
            do k = 1, mdof
...
!C*** Show Summary
        call ppohFEM_allREDUCE_R(idx_mesh,Umax,2,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Umin,2,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,Emax,3,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Emin,3,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,Smax,4,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Smin,4,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,EEmax,3,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,EEmin,3,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,ESmax,4,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,ESmin,4,hecmw_min)
...
!C*** Show Summary
        call ppohFEM_allREDUCE_R(idx_mesh,Umax,3,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Umin,3,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,Emax,6,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Emin,6,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,Smax,7,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Smin,7,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,EEmax,6,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,EEmin,6,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,ESmax,7,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,ESmin,7,hecmw_min)
        if( ppohFEM_comm_get_rank()==0 ) then
          write(ILOG,*) '##### Global Summary :Max/Min####'
          write(ILOG,1019) '//U1 ',Umax(1),Umin(1)
          write(ILOG,1019) '//U2 ',Umax(2),Umin(2)
...
!C*** Show Summary
        call ppohFEM_allREDUCE_R(idx_mesh,Umax, 6,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Umin, 6,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,Emax,14,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Emin,14,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,Smax,14,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,Smin,14,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,EEmax,14,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,EEmin,14,hecmw_min)
        call ppohFEM_allREDUCE_R(idx_mesh,ESmax,14,hecmw_max)
        call ppohFEM_allREDUCE_R(idx_mesh,ESmin,14,hecmw_min)
        if( ppohFEM_comm_get_rank()==0 ) then
          write(ILOG,*) '##### Global Summary :Max/Min####'
          write(ILOG,1019) '//U1    ',Umax(1),Umin(1)
          write(ILOG,1019) '//U2    ',Umax(2),Umin(2)
