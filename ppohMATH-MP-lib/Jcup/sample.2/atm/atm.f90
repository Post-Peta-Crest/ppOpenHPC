...
subroutine atm_init()
  ...
  call jcup_initialize(ATM, LOG_LEVEL = 0, LOG_STDERR = 0)
  ...
  call land_init_grid(is_restart())
  ...
  delta_t = get_step(ATM)

  if (is_restart()) then
    call jcup_read_restart(32, start_time)
    !call mpi_finalize(ij)
    !stop
  end if
  ...
  call land_put_initial_data(is_restart())

  step_counter = 0

  if (is_restart()) then
    open(unit = file_id, file = trim(ATM)//".mst", form = "formatted", status = "old", err= 200)
...
subroutine atm_fin()
  ...
  open(unit = file_id, file = trim(ATM)//".mst", form = "formatted", status = "replace", err = 200)
