...
module mod_config
  ...
  public :: read_conf      ! subroutine (conf_file_name)
  public :: get_start_time ! function ()
  public :: get_end_time   ! function ()
  public :: get_step       ! integer function (model_name)
  public :: is_end_step    ! logical function (now_time, end_time)
  public :: is_restart     ! logical funcion ()

  integer :: start_time(6)
  integer :: end_time(6)
  integer :: stepa, stepo, stepc
  integer :: restart = 0
...
subroutine read_conf(file_name)
  implicit none
  character(len=*), intent(IN) :: file_name
  namelist / runcnf / start_time, end_time, stepa, stepo, stepc, restart
  integer :: ctl_fid
  integer :: ierror
  ...
  open(CTL_FID,             &
       ...
       iostat=ierror)
  if(ierror/=0) then
     write(*,*) 'Cannot open PARAMETER file! ', trim(file_name)
     stop
  end if
  rewind(ctl_fid)
  read(ctl_fid,nml=runcnf)
  close(ctl_fid)
...
function get_start_time() result(time)
  implicit none
  integer :: time(6)
...
function get_end_time() result(time)
  implicit none
  integer :: time(6)
...
integer function get_step(model_name)
  use field_common, only : ATM, OCN, CHM
  implicit none
  ...
  case (CHM)
    get_step = stepc
  case default
    write(0,*) "get_step, model_name error ", trim(model_name)
    stop    
  end select
...
logical function is_end_step(now_time, end_time)
  implicit none
  integer, intent(IN) :: now_time(6), end_time(6)
  integer(kind=8) :: nowt, endt

  nowt = now_time(1)*10000000000+now_time(2)*100000000+now_time(3)*1000000+now_time(4)*10000+now_time(5)*100+now_time(6)
  endt = end_time(1)*10000000000+end_time(2)*100000000+end_time(3)*1000000+end_time(4)*10000+end_time(5)*100+end_time(6)

  is_end_step = (nowt >= endt)

end function is_end_step
