...
subroutine ocn_init()
  ...
  call jcup_initialize(OCN, LOG_LEVEL = 0, LOG_STDERR = 0)
  ...
  call ice_put_initial_data(is_restart())

  step_counter = 0

  if (is_restart()) then
    open(unit = file_id, file = trim(OCN)//".mst", form = "formatted", status = "old", err= 200)
...
subroutine ocn_fin()
  ...
  open(unit = file_id, file = trim(OCN)//".mst", form = "formatted", status = "replace", err = 200)
