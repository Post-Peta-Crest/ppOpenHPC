...
subroutine land_init_grid(is_restart)
  ...
  if (is_restart) then
    open(unit = file_id, file = trim(LAND)//".mst", form = "formatted", status = "old", err= 200)
...
200 continue
  ...
  write(0,*) "file "//trim(LAND)//".mst open error"
  stop
...
subroutine land_put_initial_data(is_restart)
  use jcup_interpolation_sample
  ...
  if (.not.is_restart) call set_and_put_data(step_counter)
...
subroutine land_fin()
  ...
  open(unit = file_id, file = trim(LAND)//".mst", form = "formatted", status = "replace", err = 200)
...
200 continue
  write(0,*) "land data output error"
