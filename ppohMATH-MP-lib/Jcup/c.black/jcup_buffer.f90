...
subroutine insert_time_buffer(tb, time)
  ...
  call put_log("insert time buffer : "//time_str)
...
subroutine delete_time_buffer(tb, time)
  ...
  call DateToTimeStr(time_str, time)
  call put_log("delete time buffer : "//time_str)
...
subroutine reset_time_buffer(tb, time, component_id)
  ...
  call DateToTimeStr(time_str, time)
  call put_log("reset time buffer : "//time_str)
...
subroutine reset_past_time_buffer(tb, current_time, component_id)
  ...
  do 
    if (tb%time < current_time)  then
      ...
      call put_log("search and reset time buffer : "//time_str)
      if (tb%num_of_data > 0) then
        call reset_comp_data_buffer(tb%dt_start,component_id)
        tb%num_of_data = get_num_of_using_data_buffer(tb%dt_start)
      end if
...
subroutine search_time_buffer(tb, time, insert_flag)
  ...
  do 
    if (tb%time == time)  then
      tb%is_using = .true.
      call put_log("search time buffer : same time found : "//time_str)
      return
    end if
    tb => tb%next_ptr
    if (associated(tb, start_time)) exit
  ...
  do 
    if (.not.tb%is_using) then
      ...
      tb%time = time
      tb%is_using = .true.
      call put_log("search time buffer : usable time found : "//old_time_str//" : "//time_str)
      return
    end if
    tb => tb%next_ptr
    if (associated(tb, start_time)) exit
  ...
  if (is_insert) then
    ...
    call put_log("search time buffer !!!! No such time "//time_str)
  end if
...
subroutine write_time_buffer_info(tb)
  ...
  write(log_str,'(" Time buffer info : ",A15,L,I5)') time_str, tb%is_using, tb%num_of_data
...
subroutine get_send_data_double_1d(dt, time, component_id, data_id, name)
  ...
  call DateToTimeStr(time_str, time)
  call put_log(&
    & "get_send_data_double_1d : get data : name = "//trim(name)//", data id = "//trim(IntToStr(data_id))//", time = "//time_str)
  call search_time_buffer(send_buffer, time)
