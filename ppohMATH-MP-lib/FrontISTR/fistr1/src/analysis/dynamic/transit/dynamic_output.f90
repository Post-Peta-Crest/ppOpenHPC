...
!C-- stress
    ...
    if( hecMESH%my_rank==0 ) then ! Open file and output header
...
!        if(fstrDYNAMIC%i_step==0) then
    ...
    fstrDYNAMIC%strainEnergy  = 0.0d0
    do ii = 1, hecMESH%nn_internal*hecMESH%n_dof
      fstrDYNAMIC%kineticEnergy = fstrDYNAMIC%kineticEnergy &
      + 0.5d0 * myEIG%mass(ii) * fstrDYNAMIC%VEL(ii,idx) * fstrDYNAMIC%VEL(ii,idx)
      fstrDYNAMIC%strainEnergy  = fstrDYNAMIC%strainEnergy  &
      + 0.5d0 * fstrSOLID%QFORCE(ii) * fstrDYNAMIC%DISP(ii,idx)
    enddo
!        endif

     call hecmw_allREDUCE_R1(hecMESH,fstrDYNAMIC%kineticEnergy,HECMW_SUM)
     call hecmw_allREDUCE_R1(hecMESH,fstrDYNAMIC%strainEnergy,HECMW_SUM)

     fstrDYNAMIC%totalEnergy = fstrDYNAMIC%kineticEnergy + fstrDYNAMIC%strainEnergy

    if( hecMESH%my_rank==0 ) then ! write energy
        write( fstrDYNAMIC%dynamic_IW7, '(i10,1pe13.4e3,1p3e16.4e3)' ) &
               fstrDYNAMIC%i_step, &
               fstrDYNAMIC%t_curr, &
               fstrDYNAMIC%kineticEnergy, &
               fstrDYNAMIC%strainEnergy, &
               fstrDYNAMIC%totalEnergy
