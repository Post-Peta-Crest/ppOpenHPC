...
!C    INIT. HECMW-FEM process's
!C
      subroutine hecmw_init_ppohMATHMP(mpi_comm)
      integer, intent(IN) :: mpi_comm
      character(len=HECMW_FILENAME_LEN):: ctrlfile = "hecmw_ctrl.dat"

        call hecmw_init_ex_ppohMATHMP(ctrlfile, mpi_comm)

      end subroutine hecmw_init_ppohMATHMP
...
!C    INIT. HECMW-FEM process's
!C
      subroutine hecmw_init_ex_ppohMATHMP(ctrlfile, mpi_comm)
      character(len=HECMW_FILENAME_LEN):: ctrlfile
      integer, intent(IN) :: mpi_comm
      integer(kind=kint) :: ierr

      write(0,*) "hecmw_init_ex_ppohMATHMP ", mpi_comm, MPI_COMM_WORLD     
      
      call MPI_COMM_SIZE (mpi_comm, hecmw_PETOT, ierr)
      call MPI_COMM_RANK (mpi_comm, hecmw_rank,  ierr)
      call MPI_COMM_DUP  (mpi_comm, hecmw_comm,  ierr)
      call MPI_COMM_GROUP(mpi_comm, hecmw_group, ierr)
...
!      endif
      ...
      end subroutine hecmw_init_ex_ppohMATHMP
