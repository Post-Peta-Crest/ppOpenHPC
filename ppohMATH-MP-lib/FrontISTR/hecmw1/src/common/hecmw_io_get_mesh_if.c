...
void
hecmw_get_mesh_init_if_jcup(char *name_ID, int *err, int *my_comm, int len)
{
	char cname[HECMW_FILENAME_LEN+1];
...
//        mesh->HECMW_COMM = *my_comm ; 
//        printf("hecmw_get_mesh_init_if_jcup, comm = %d\n", *my_comm) ; 
...
void
hecmw_get_mesh_init_if_jcup_(char *name_ID, int *err, int *my_comm, int len)
{
  hecmw_get_mesh_init_if_jcup(name_ID, err, my_comm, len);
}



void
hecmw_get_mesh_init_if_jcup__(char *name_ID, int *err, int *my_comm, int len)
{
  hecmw_get_mesh_init_if_jcup(name_ID, err, my_comm, len);
}
