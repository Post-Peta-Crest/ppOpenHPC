...
private:
	...
	bool FittingF;						// Fittingが使用されているかどうか？
										...
										// ram()など）
	char Comment;
										// Fortranでリージョン内にcallがある場合に追加
										// サブルーチンが複写され、_OATの名前で使われる。

	void MakeControlCode_Fortran(FILE *fpAdd = NULL);
	void MakeControlCode_C();
	void MakeSrcCode_Fortran(FILE *fpAdd = NULL,int tokSPos = -1,int tokEPos = -1);
	void MakeSrcCode_Fortran77();
	void MakeSrcCode_C();
	int AddIncludePosList(TList *InsertIncludePosList);
	void AddUsePosList(TList *InsertUsePosList);
	void OutputModuleStart(FILE *fp);
	void CheckAndDeleteLocalValDefine();
	void Make_Call_SubroutineNameInRegionList();
	void Output_Call_SubroutineNameInRegionList();

public:
	TList *TuneRegionList;
	TStringList *Call_SubroutineNameInRegionList;
