...
subroutine ppohAMRFDM_addGoct(iLv,st_param,st_meshset,st_comm_info)
  ...
  st_meshset%MinID(4,iLv+1)=max(st_meshset%MaxID(3,st_param%LvMax),st_meshset%MaxID(4,iLv))
  index2=st_meshset%MinID(4,iLv+1)
  ...
  do ID=3,4
     if(st_meshset%MinID(ID,iLv)<st_meshset%MaxID(ID,iLv)) then
        ...
        do index=st_meshset%MinID(ID,iLv)+1,st_meshset%MaxID(ID,iLv)
           if(ID==3) then
              add(index)=add(index-1)+st_meshset%rfnoct3(index-1)
           else
              add(index)=add(index-1)+st_meshset%rfnoct4(index-1)
           endif
        enddo
        add(:)=add(:)+index2
        !$omp parallel do default(none) &
        !$omp& private(index,p0,octLv,Nint,MrtN,bndry,n1,n2,ix,iy,iz,iPOS,iFLG,ich) &
        ...
        do index=st_meshset%MinID(ID,iLv),st_meshset%MaxID(ID,iLv)
           ...
           if(p0%iFLG(2)==2) then
              ...
              do ich=1,8
                 index2=index2+1
                 if(index2>size(st_meshset%GMesh)) then
                    print *,"GMesh overflow, size(GMesh)=",size(st_meshset%GMesh),st_comm_info%rank
                 ...
                 ! -- Input informaions into a target variable GMesh(:) --
                 st_meshset%GMesh(index2)=&
                 ...
                 if(ich==1) then
                    p0%octCh1 => st_meshset%GMesh(index2)
                 else if(ich==2) then
                    p0%octCh2 => st_meshset%GMesh(index2)
                 else if(ich==3) then
                    p0%octCh3 => st_meshset%GMesh(index2)
                 else if(ich==4) then
                    p0%octCh4 => st_meshset%GMesh(index2)
                 else if(ich==5) then
                    p0%octCh5 => st_meshset%GMesh(index2)
                 else if(ich==6) then
                    p0%octCh6 => st_meshset%GMesh(index2)
                 else if(ich==7) then
                    p0%octCh7 => st_meshset%GMesh(index2)
                 else if(ich==8) then 
                    p0%octCh8 => st_meshset%GMesh(index2)
                 endif
              enddo
           endif
        enddo
     ...
     ! -- add GMeshID for new octs --
     if(index2==st_meshset%MinID(4,iLv+1)) then 
        st_meshset%MinID(4,iLv+1)=st_meshset%MaxID(4,iLv)
        st_meshset%MaxID(4,iLv+1)=st_meshset%MaxID(4,iLv)
     else
        st_meshset%MinID(4,iLv+1)=st_meshset%MaxID(4,iLv)+1
        st_meshset%MaxID(4,iLv+1)=index2
     endif
  enddo
  ! -- interpolate physical variables for new octs --
  do ID=3,4
     call ppohAMRFDM_interpolate(iLv,2,2,ID,st_param,st_meshset)
  enddo
  ! -- reset iFLG(2) --
  do ID=3,4
  ...
  enddo
  deallocate(st_meshset%rfnoct3); deallocate(st_meshset%rfnoct4)
  return
end subroutine ppohAMRFDM_addGoct
