...
subroutine ppohAMRFDM_load_mesh(iLv,index,p0,st_meshset)
  use m_ppohAMRFDM_util
  implicit none
  integer(kind=ppohAMRFDM_kint),intent(in)::iLv,index
  type(st_ppohAMRFDM_octset),intent(out)::p0
  type(st_ppohAMRFDM_meshset),intent(in)::st_meshset
  p0=st_meshset%Mesh(st_meshset%getID(iLv)%IDsq(index))
  return
end subroutine ppohAMRFDM_load_mesh
