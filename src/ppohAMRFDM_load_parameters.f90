...
subroutine ppohAMRFDM_load_parameters(iLv,dt,dx,nID,st_param)
  use m_ppohAMRFDM_util
  implicit none
  integer(kind=ppohAMRFDM_kint),intent(in)::iLv
  integer(kind=ppohAMRFDM_kint),intent(out)::nID
  real(kind=ppohAMRFDM_kdbl),intent(out)::dt,dx(1:3)
  type(st_ppohAMRFDM_param)::st_param
  nID=st_param%nID(iLv)
  dt=st_param%dt*(0.5d0**iLv)
  dx(:)=st_param%dx(:)*(0.5d0**iLv)
  return
end subroutine ppohAMRFDM_load_parameters
