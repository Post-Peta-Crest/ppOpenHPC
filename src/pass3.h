...
enum Enum_ScType  // 自動チューニング種類
{
	...
	// Bind Add 2014/03/14
	sct_Bind,					// !OAT$ bind ATexec arguments start
	...
	sct_Option,		 	// option Add 2016/02/29
	sct_variedD,
};
...
enum Enum_ScAction  // 機能
{
	...
	sca_bind_atexec_arg_start, 			// bind アクション定義開始
	sca_bind_atexec_arg_end, 			// bind アクション定義終了
...
public:
	...
	TStringList *bindValNameList;		// bindでの置換変数名リスト。NewとOldが交互に入る。
...
//d-spline用追加部分 ここまで
	...
	void GetBindArgument(TList *TokenList,int argumentEndPos);
	string ReplaseBindValName(string argStr);
};
