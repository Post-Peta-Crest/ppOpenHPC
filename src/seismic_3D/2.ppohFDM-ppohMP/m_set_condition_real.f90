...
contains
  ...
  !
    ...
    read(7,*) MST
    print *, MST
    print *,'Medium parameter [NO. RO VP VS]'
    do i=1, MST
       read(7, *) tmp1, RO1(i), VP1(i), VS1(i)
       print *,tmp1, RO1(i), VP1(i), VS1(i)
    end do
    close(7)
  end subroutine ppohFDM_set_medium_param

  subroutine ppohFDM_read_geom_file_name()
    integer :: i
    character(len=80) filename

    filename="geometry_real.dat"
    print *,'Reading Geometry list'
    open(7,file=filename,status='old')
    read(7,*) geom_num
    do i = 1, geom_num
       write(geometrylist(i), '("MODEL"i0.2".geom")') i-1
       print *, geometrylist(i)
    end do
    close(7)
  end subroutine ppohFDM_read_geom_file_name

  subroutine ppohFDM_read_geom_data()
  !
  ! Sets up the test medium for the subsurface structure.
  !
    integer :: m, j, k
    ...
    call ppohFDM_read_geom_file_name()
    print *,'Reading Geometry DATA'
    do m=1, geom_num
       open(7+m,file=geometrylist(m),status='old')
       print *,'Open Geometry DATA'
       do K=0, NY+1
          do j=0, NX+1
             read(7+m,*) tmp3, tmp3, tmp3, tmp3, tmp3, tmp3, ZDEP(m,j,k)
             ZDEP(m,j,k)=ZDEP(m,j,k)*0.001
          end do
        end do
       print *,'CLOSE Geometry DATA'
       close(7+m)
    end do
  end subroutine ppohFDM_read_geom_data
       ...
       do I = 0, NXP+1
          do K = 0, NZP+1
             do M=1, geom_num
                ...
                if(ZDEP(M,ii,jj)>0)then
                   kz = int (ZDEP(M,ii,jj)/Dz+0.5)+KFSZA(ii,jj)
                else
                   kz = int (ZDEP(M,ii,jj)/Dz-0.5)+KFSZA(ii,jj)
                end if
                ...
                if(kk <= KFSZA(ii,jj)) then
                ...
                else if(kza(M,ii,jj) < kk .and. kk <= kza(M+1,ii,jj)) then
                   ...
                   RIG  (I,J,K) = RO*VS**2
                   LAM  (I,J,K) = RO*VP**2 - 2*RO*VS**2
                else if(kza(MST+1,ii,jj) < kk) then
                   RO = RO1(MST)
                   VP = VP1(MST)
                   VS = VS1(MST)
                   DEN  (I,J,K) = RO
