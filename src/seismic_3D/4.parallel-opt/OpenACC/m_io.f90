...
contains
  ...
  !
    ...
    !$acc data pcopyin(VX,VY,VZ,ISTX,ISTY,ISTZ)
    ...
    if( it == 1 ) then
       ...
       !$acc enter data copyin(VXALL,VYALL,VZALL,UXALL,UYALL,UZALL,ux,uy,uz)
       
       !$acc kernels pcopy(UXALL,UYALL,UZALL,ux,uy,uz,vx0,vy0,vz0)& 
       !$acc& pcopyin(ISTX,ISTY,ISTZ,VX,VY,VZ)
       ...
       !$acc kernels pcopy(UX,UY,UZ,vx0,vy0,vz0)& 
       !$acc& pcopyin(ISTX,ISTY,ISTZ,VX,VY,VZ)
       !$acc loop independent gang vector
       do ns = 1, NST                                                 
          istax  =  ISTX ( NS )                                           
    ...
    if( mod( it, NSKIP ) == 0 ) then
       it1 = it / nskip
       !$acc  kernels pcopy(VXALL,VYALL,VZALL,UXALL,UYALL,UZALL) &
       !$acc& pcopyin(VX,VY,VZ,UX,UY,UZ,ISTX,ISTY,ISTZ)
       !$acc loop independent gang vector
       do ns=1, NST
       ...
       !$acc update host(VXALL,VYALL,VZALL,UXALL,UYALL,UZALL)
...
!       write( io ) UXALL, UYALL, UZALL
  ...
  !
    ...
    if( it == 1 ) then
       ...
       !$acc kernels pcopy(vzsnap,vhsnap) pcopyin(rig,den,kfsz)
       ...
       !$acc enter data pcopyin(vxsnap,vysnap,vzsnap,vzsnap,vhsnap)

    end if
    
    
    !! Preparation at every time step
    !$acc kernels pcopy(VXSNAP,VYSNAP,VZSNAP,VHSNAP) pcopyin(VX,VY,VZ,KFSZ)
    !$acc loop gang vector(8)
    do j = 1, NYS
       !$acc loop gang vector(32)
       do i = 1, NXS
  ...
  subroutine ppohFDM_io_write_sps( it, io )
    ...
    if( it == 1 ) then
       ...
       !$acc kernels pcopy(VPSNAP,VSSNAP) pcopyin(RIG,DEN,KFSZ)
       !$acc loop gang vector(8)
       do J = 1, NYS
          !$acc loop gang vector(32)
          do I = 1, NXS
       ...
       !$acc update host(VPSNAP,VSSNAP)
       write ( io, rec=ISNAP ) VPSNAP
       isnap = 2
       write ( io, rec=ISNAP ) VSSNAP
       
       !$acc enter data pcopyin(VPSNAP,VSSNAP)
    ...
    if( mod( it, NWRITE ) == 0 ) then
       ...
       !$acc  kernels pcopyout(VPSNAP,VSSNAP)&
       !$acc& pcopyin(KFSZ,DXVX,DYVY,DZVZ,DXVY,DYVX,DYVZ,DZVY,DZVX,DXVZ)
       !$acc loop gang vector(8)
       do j = 1, NYS
          !$acc loop gang vector(32)
          do i = 1, NXS
  ...
  !
    ...
    if( it == 1 ) then
       ...
       !$acc kernels pcopy(VPSNAP,VSSNAP) pcopyin(RIG,DEN)
       !$acc loop gang vector(8)
       do j = 1, NYS
          !$acc loop gang vector(32)
          do i = 1, NXS
    ...
    if( mod( it, NWRITE ) == 0 ) then
       ...
       !! Prepare P- and S- wave amplitude
       !$acc kernels pcopy(VPSNAP,VSSNAP) &
       !$acc& pcopyin(DXVX,DYVY,DZVZ,DXVY,DYVX,DYVZ,DZVY,DZVX,DXVZ)
       !$acc loop gang vector(8)
       do j = 1, NYS
          !$acc loop gang vector(32)
          do i = 1, NXS
