...
!
! Moment-tensor point source implementation based on either the stress-drop formulation
!
  use ppohFDM_stdio
  use ppohFDM_param
  implicit none
...
contains
  ...
  !
    ...
    if( is_src ) then
       ...
       if( T <= FTMAX ) then
          ...
          !$acc kernels pcopy(SXX,SYY,SZZ,SYZ,SXZ,SXY) pcopyin(STIME) async(0)
  ...
  !
    ...
    if( is_src ) then
       ...
       !$acc kernels pcopy(VX,VY,VZ) pcopyin(STIME,DEN) async(0)
