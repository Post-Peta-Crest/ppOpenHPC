...
!
  ...
  !$acc  data&
  !$acc& pcopy(VX,VY,VZ,SXX,SYY,SZZ,SYZ,SXZ,SXY) &
  !$acc& pcopy(DXVX,DYVX,DZVX,DXVY,DYVY,DZVY,DXVZ,DYVZ,DZVZ) &
  !$acc& pcopy(DXSXX,DYSXY,DZSXZ,DXSXY,DYSYY,DZSYZ,DXSXZ,DYSYZ,DZSZZ) &
  !$acc& pcopyin(DEN,RIG,LAM)&
...
contains
  ...
  !
    ...
    if( .not. is_fs ) then 
       ...
       !$acc kernels pcopyin(VX,VY,VZ,KFSZ)
       !$acc loop gang reduction(max:xmax0,ymax0,zmax0)
       do j=1, NYP, NYD
          !$acc loop gang vector reduction(max:xmax0,ymax0,zmax0)
          do i=1, NXP, NXD
             xmax0 = max( xmax0, abs( VX( i,j,KFSZ(i,j)+1 ) ) )
             ymax0 = max( ymax0, abs( VY( i,j,KFSZ(i,j)+1 ) ) )
             zmax0 = max( zmax0, abs( VZ( i,j,KFSZ(i,j)+1 ) ) )
          end do
       end do
       !$acc end kernels
    end if
  ...
  !
    ...
    !! X dir
    if( idx == 0 ) then
       !$acc kernels pcopy(DXVX,DXVY,DXVZ) pcopyin(VX,VY,VZ) async(0)
    ...
    if( idy == 0 ) then ! Shallowmost
       !$acc kernels pcopy(DYVX,DYVY,DYVZ) pcopyin(VX,VY,VZ) async(0)
       !$acc loop gang vector(8)
       do K=1, NZP
          !$acc loop gang vector(32)
          do I=1, NXP
    ...
    if( idz == 0 ) then ! Shallowmost
       !$acc kernels pcopy(DZVZ,DZVX,DZVY) pcopyin(VX,VY,VZ) async(0)
       !$acc loop gang vector(8)
       do J=1, NYP
          !$acc loop gang vector(32)
          do I=1, NXP
