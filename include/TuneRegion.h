...
//    int UnrollFromValue[32];		// アンロールの値の変化
//    int UnrollToValue[32];
//    int UnrollCaseCount;			// 全アンロールのCase数

//    bool RegionStartF;  // リージョン開始中フラグ
//    TScript *ScUnroll;	// unroll要求スクリプト中 (region start)
//    TList *PreProList;	// PrePro要求のリスト
	...
	void OutputSetParamCode_Fortran(FILE *fp,int TempLineNo);
...
//	void TTuneRegion::OutputSrcCode(FILE *fp,AnsiString IndentStr,bool InCaseF);
	void OutputSrcCode(FILE *fp,string IndentStr,int OutMode);
	...
	int OutputUnroll_DoBlock_C(FILE *fp,int TokPos,unsigned int UseDoRefValBits,int RefValIdx);
...
//    AnsiString TTuneRegion::GetPrePostSubregionStr(int Mode);
	...
	void OutputFusionDo(string FusionValName,FILE *fp,int DoNest,int FusionIdx);
